...
### Guardrails и память диалога

- В `bot.py` добавлены guardrails для ограничения тем разрешенной областью: услуги, портфолио, проекты/кейсы, опыт, сроки, цены и контакты.
- Добавлены вежливые локальные отказы для посторонних тем и запросов на раскрытие системного промпта, скрытых инструкций, секретов, токенов или служебной логики.
- В системный промпт Deepseek добавлены инструкции игнорировать prompt injection, не менять роль и не выходить за разрешенную область.
- Добавлена in-memory память диалога по паре чат/пользователь с ограничением истории до `MAX_HISTORY_MESSAGES` сообщений.
- Добавлена команда `/reset`, которая очищает память текущего диалога.

### Валидация

- `python -m py_compile bot.py` → успешно, синтаксических ошибок нет.
- Локальная проверка функций guardrails и памяти диалога через `python -c "..."` → успешно: опасные/посторонние запросы получают отказ, история обрезается до заданного лимита.

## 2026-08-04—2026-08-05

### Витрина услуг, SQLite и оформление заказа

- Добавлены витрина услуг и SQLite-хранилище: в коммите `1eed6da` изменены `bot.py`, `README.md` и `.gitignore`, создан `database.py`.
- Добавлено оформление заказа из корзины: в коммите `55cf769` изменены `bot.py` и `database.py`.

### Проверка состояния

- На момент обновления журнала незакоммиченных изменений в рабочей папке нет (`git status --short`).

## 2026-08-06

### Автотесты корзины, заказов и оплаты

- Добавлены pytest-автотесты в `tests/test_cart_orders_and_payments.py` для корзины, оформления заказа, сохранности данных SQLite после перезапуска и статуса оплаты.
- Stripe и Telegram в тестах имитируются моками; реальные сетевые запросы не выполняются.
- `pytest` добавлен в `requirements.txt`.

### Валидация

- `python -m pytest -q` → успешно: `4 passed in 1.18s`.
- `python -m py_compile bot.py database.py tests\test_cart_orders_and_payments.py` → успешно, синтаксических ошибок нет.
